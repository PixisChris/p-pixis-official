...
parcelRequire = (function (modules, cache, entry, globalName) {
  ...
  return newRequire;
})({"cuGp":[function(require,module,exports) {
// USAGE
...
function translate() {
  document.querySelectorAll('[data-i18n]').forEach(function (el) {
    var i18nDotNotationKey = el.getAttribute('data-i18n');

    if (i18nDotNotationKey) {
      // 將 data-i18n='a.b.c' 拉出 window.locale object 中對應的翻譯文字
      var translation = i18nDotNotationKey.split('.').reduce(index, window.locale);
  ...
  document.querySelectorAll("[data-i18n-show-if=".concat(localStorage.getItem('pixis_lang'), "]")).forEach(function (el) {
    console.log(el);
    el.hidden = false;
  });
  document.querySelectorAll("[data-i18n-show-if]:not([data-i18n-show-if=".concat(localStorage.getItem('pixis_lang'), "])")).forEach(function (el) {
    console.log(el);
    el.hidden = true;
  });
  document.querySelectorAll('input[required]').forEach(function (requiredInput) {
    var requiredMessage = "common.validations.required".split(".").reduce(index, window.locale);
    var emailInvalidMessage = "common.validations.email".split(".").reduce(index, window.locale);
    var generalInvalidMessage = "common.validations.invalid".split(".").reduce(index, window.locale);
  ...
  document.querySelectorAll('select[required]').forEach(function (requiredInput) {
    var requiredMessage = "common.validations.required".split(".").reduce(index, window.locale);
    var generalInvalidMessage = "common.validations.invalid".split(".").reduce(index, window.locale);
    requiredInput.addEventListener("invalid", function (event) {
  ...
  if (document.querySelector('#home_video video.pc')) {
    var videoPc = document.querySelector('#home_video video.pc');
    videoPc.querySelectorAll("source").forEach(function (el) {
      el.remove();
    });
    var sourcePc = document.createElement('source');
    var urlPc = "";
  ...
  if (document.querySelector('#home_video video.m')) {
    var videoM = document.querySelector('#home_video video.m');
    videoM.querySelectorAll("source").forEach(function (el) {
      el.remove();
    });
...
function detectLocale() {
  var params = new URLSearchParams(window.location.search); // 根據 localStorage 判斷
...
function _getLangBtnHTML() {
  var locale = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : localStorage.getItem('pixis_lang');
  return "<a href=\"javascript:;\" data-lang=\"tw\" class=\"lang-btn tw ".concat(locale === 'tw' ? 'is-on' : '', "\">\u7E41\u4E2D</a>\n\t\t\t\t\t<a href=\"javascript:;\" data-lang=\"en\" class=\"lang-btn en ").concat(locale === 'en' ? 'is-on' : '', "\">EN</a>");
}

function enableNavTranslate() {
  var desktopNavMenu = document.querySelector('#nav .nav-menu');
  var mobileNavMenu = document.querySelector('#nav-mobile .nav-content'); // 塞入桌機版、手機版的語系按鈕 html
  ...
  langBtns.forEach(function (el) {
    el.addEventListener('click', function () {
      // 取語系
      window.locale = window[el.getAttribute('data-lang')]; // 切換樣式

      langBtns.forEach(function (el) {
        return el.classList.toggle('is-on');
      }); // 儲存

      localStorage.setItem('pixis_lang', el.getAttribute('data-lang')); // 翻譯

      translate();
    });
  });
}
},{}],"fLAN":[function(require,module,exports) {
...
},{}],"HLO8":[function(require,module,exports) {
document.addEventListener('DOMContentLoaded', function () {
...
},{}],"e6dU":[function(require,module,exports) {
"use strict";
    ...
    i && (t.preventDefault(), function (t) {
      ...
      var c = function (t) {
        return Math.floor(t.getBoundingClientRect().top);
          ...
          s = null,
          h = function h(t) {
        if (m) return document.documentElement.scrollTop = d, a.focus(), void window.history.pushState("", "", i);
        (l == d || t - s >= o) && (m = !0);
        var e = r((t - s) / o),
            n = l + (d - l) * e;
...
},{}],"U0fG":[function(require,module,exports) {
"use strict";
...
exports.loadHTMLById = exports.docReady = exports.toggleClassName = void 0;

var toggleClassName = function toggleClassName(el, className) {
...
var loadHTMLById = function loadHTMLById(myDivId, url) {
  var xmlhttp;
...
exports.loadHTMLById = loadHTMLById;
...
require("./_white-paper");

require("./_contact");

var _scrolltoanchorEsm = _interopRequireDefault(require("./scrolltoanchor.esm.js"));

var _utils = require("./_utils");

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var smoothScroll = new _scrolltoanchorEsm.default({
  offset: 200
});
window.addEventListener('load', function () {
...
(0, _utils.docReady)(function () {
  ...
  (0, _utils.loadHTMLById)('nav-components', 'nav.html');
  (0, _utils.loadHTMLById)('footer-components', 'footer.html');
  (_document$querySelect = document.querySelector('#float-banner .close-btn')) === null || _document$querySelect === void 0 ? void 0 : _document$querySelect.addEventListener('click', function () {
    (0, _utils.toggleClassName)('#float-banner', 'hide');
  });
  setHomePageScreensEffect();

  var scrollHandler = function scrollHandler() {
    var wH = window.innerHeight;
    var yOff = window.pageYOffset + wH;
  ...
  if (document.body.id === 'function-page') {
    ...
    // 左上角小選單
    (_document$querySelect2 = document.querySelector('.function-select')) === null || _document$querySelect2 === void 0 ? void 0 : _document$querySelect2.addEventListener('click', function (e) {
      e.stopPropagation();
      (0, _utils.toggleClassName)('header .function-options', 'active');
    });
...
var setHomePageScreensEffect = function setHomePageScreensEffect() {
  // screens start
  var screenNodeList = document.querySelectorAll('.screen-preview');
  var screens = Array.apply(null, screenNodeList);
  var screenContentNodeList = document.querySelectorAll('.function-wrapper');
  var screenContents = Array.apply(null, screenContentNodeList);
  var dataScreens = Array.apply(null, document.querySelectorAll('[data-screen]'));
  var screenIndex = 1;
  setInterval(function () {
    var _document$querySelect3;

    (_document$querySelect3 = document.querySelector('.bar-body')) === null || _document$querySelect3 === void 0 ? void 0 : _document$querySelect3.classList.add('move');
    dataScreens.forEach(function (dataScreen) {
      if (dataScreen.dataset.screen == screenIndex) {
        dataScreen === null || dataScreen === void 0 ? void 0 : dataScreen.classList.add('active');
      } else {
        dataScreen === null || dataScreen === void 0 ? void 0 : dataScreen.classList.remove('active');
      }
    });
    screenIndex = screenIndex + 1 > 6 ? 1 : screenIndex + 1;
  }, 5000);
  screens.forEach(function (screen) {
    var screenNum = screen.dataset.screen;
    screen.addEventListener('click', function () {
      screens.forEach(function (s) {
        if (s.dataset.screen === screenNum) {
          s === null || s === void 0 ? void 0 : s.classList.add('active');
        } else {
          s === null || s === void 0 ? void 0 : s.classList.remove('active');
        }
      });
      screenContents.forEach(function (s) {
        if (s.dataset.screen === screenNum) {
          s === null || s === void 0 ? void 0 : s.classList.add('active');
        } else {
          s === null || s === void 0 ? void 0 : s.classList.remove('active');
...
var setHomePageWelcomeEffect = function setHomePageWelcomeEffect(yOff, wH) {
  var scrollAnims = document.querySelectorAll('.scroll-anim');
  scrollAnims.forEach(function (scrollAnim) {
    var scrollPos = scrollAnim.offsetTop + wH / 2;

    if (yOff >= scrollPos) {
      scrollAnim === null || scrollAnim === void 0 ? void 0 : scrollAnim.classList.add('scroll-anim-show');
      scrollAnim === null || scrollAnim === void 0 ? void 0 : scrollAnim.classList.add('scroll-anim-delay');
      setTimeout(function () {
        scrollAnim === null || scrollAnim === void 0 ? void 0 : scrollAnim.classList.remove('scroll-anim-delay');
      }, 1000);
    }
  });
};

var setFnPageEffect = function setFnPageEffect(yOff, wH) {
  var fnContainers = document.querySelectorAll('[data-scroll="fn"]');
  fnContainers.forEach(function (fnContainer) {
    var layout = fnContainer.dataset.scrollLayout; // left or right

    var fnNav = fnContainer.querySelector(".function-nav");
    var fnMainScreen = fnContainer.querySelector('.main-screen');
    var scrollBaseline = fnContainer.querySelector('.scroll-baseline');
    var scrollEndline = fnContainer.querySelector('.scroll-endline');
    var fnContent = fnContainer.querySelector('.function-content'); // 功能頁文字區塊

    var screenStart = scrollBaseline.offsetTop + fnMainScreen.offsetHeight / 2;
    var screenMid = fnMainScreen.offsetTop + fnMainScreen.offsetHeight / 3 * 2;
    var screenEnd = fnMainScreen.offsetTop + fnMainScreen.offsetHeight;
    var pageEnd = scrollEndline.offsetTop;

    if (yOff >= screenMid) {
      var perc = (yOff - screenMid) / (screenEnd - screenMid) > 1 ? 1 : (yOff - screenMid) / (screenEnd - screenMid);

      if (yOff <= pageEnd + 500) {
        fnNav.style.opacity = perc;
        fnNav.style.transform = "translateY(".concat(10 - perc * 10, "%)");
    ...
    if (yOff >= screenStart) {
      var _perc = (yOff - screenStart) / (screenEnd - screenStart) > 1 ? 1 : (yOff - screenStart) / (screenEnd - screenStart); // console.log( (yOff - screenStart) / (screenEnd - screenStart) )


      if (yOff <= pageEnd) {
        var translateX = layout === 'left' ? "".concat(50 * _perc * window.innerWidth * 0.01 - 5 * _perc * window.innerWidth * 0.01) : "".concat(-50 * _perc * window.innerWidth * 0.01 + 5 * _perc * window.innerWidth * 0.01);
        var scaleValue = 1 - 0.4 * _perc; // 因為 sticky 的更動不完全可配合原算法，因此當功能頁文字區塊出現在瀏覽畫面內時，就將 scale transition 至 0.6

        if (fnContent.getBoundingClientRect().top < window.innerHeight) {
          scaleValue = 0.6;
        }

        fnMainScreen.style.transform = "scale(".concat(scaleValue, ") translate3d(").concat(Math.floor(translateX * 10 * 0.83) / 10, "px, 0, 0)"); // 將影片改為 sticky 固定，不使用 translateY 持續計算
    ...
    var fnNavItems = fnContainer.querySelectorAll('.function-list .item');
    fnNavItems.forEach(function (fnNavItem) {
      var anchor = fnNavItem.dataset.scroll;
      var itemContent = fnContainer.querySelector(".item[data-scroll=\"".concat(anchor, "\"]"));
      var itemContentHight = itemContent.offsetHeight;
      var positionStart = itemContent.offsetTop + wH - 230;
      var positionEnd = positionStart + itemContentHight;
...
},{"./_i18n":"cuGp","./_white-paper":"fLAN","./_contact":"HLO8","./scrolltoanchor.esm.js":"e6dU","./_utils":"U0fG"}]},{},["NvjX"], null)
//# sourceMappingURL=../js.19ec0627.js.map